import { asyncHandler } from "../utils/asyncHandler.js";
   ...
   const request = asyncHandler(async (req, res, next) => {
     try {
       ...
       if (isLocalhost) {
         // Return error instructing frontend to handle localhost via CLI proxy
         return res.status(400).json({
           statusCode: 400,
           statusText: "Bad Request",
           responseTime: 0,
           headers: {},
           body: "Localhost URLs must be proxied via CLI. Run 'npx api-explorer-cli' and send request to http://localhost:9999/proxy from the frontend.",
           size: 0,
         });
       } else {
